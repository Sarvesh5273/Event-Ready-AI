...
- **Live Mode also generates one bonus video clip, for the top outfit only.** Once all 3 Apparel VTO tasks are terminal, `liveProcessing.ts` picks the highest-confidence outfit whose try-on actually succeeded (`pickRecommendedCatalogItemId`, shared with the report builder so the two always agree) and runs it through the YouCam AI Image-to-Video Generator (`lib/youcam/imageToVideo.ts`), sourcing straight from that outfit's already-hosted VTO result image URL (no re-upload). This delays the "ready" transition until the video task also reaches a terminal state (success/error/skipped). Demo Mode never runs this — `report.video` is always `null` there. Only 1 clip is generated per session (not 3) to bound YouCam API unit cost.
...
- **Second flow: "check your own garment" (`garmentSource: "custom"`).** Live Mode only, no Demo Mode equivalent. User uploads selfie + full-body + a garment photo (skips the preferences screen entirely — no style vibe/budget). The garment's category (full_body/upper_body/lower_body) is only collected because YouCam's VTO API requires it, never used for scoring. Dominant garment color is extracted locally via `jimp` (`lib/youcam/garmentColor.ts`, no extra YouCam call) and mapped to the nearest catalog `ColorFamily`/`Undertone`, then scored with the *same* rule ladder as catalog items (`lib/scoring/skinColorFit.ts`'s `computeSkinColorFit`, shared by both flows) via `lib/scoring/customGarmentScore.ts`. Reuses the same single bonus try-on video step as the catalog flow — no separate cost model.
- **Custom-garment photo bytes never go in the session token.** They're stored server-side in `lib/session/garmentImageStore.ts` (own 30-min TTL, separate from `liveUploadStore` which is cleared once the VTO pipeline finishes) and served back via `GET /sessions/:id/garment-image?token=...` for direct `<img src>` use. This endpoint is intentionally outside the OpenAPI/Orval-typed surface. Do not put raw/base64 image bytes into `SessionPayload`/the signed token — it's echoed back as an HTTP request header on every poll, and a base64'd photo is large enough to trip HTTP 431 (Request Header Fields Too Large).
